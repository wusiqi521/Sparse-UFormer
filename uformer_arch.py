...
class MSFN(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, bias=False,act_layer=nn.GELU, drop=0.):
        super().__init__()
...
class STBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        self.num_heads = num_heads
        self.Attention = Attention(dim=dim, num_heads=self.num_heads)
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer,
                       drop=drop) if token_mlp == 'ffn' else MSFN(dim, mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        self.blocks = nn.ModuleList([
            STBlock(dim=dim, input_resolution=input_resolution,
...
class Uformer(nn.Module):
    def __init__(self, img_size=128, img_ch=3, output_ch=3,
                 embed_dim=32, depths=[4, 6, 6, 8, 8, 8, 6, 6, 4], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
